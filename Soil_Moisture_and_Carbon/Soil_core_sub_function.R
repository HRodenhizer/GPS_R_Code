...
f <- Vectorize(function( depth0, depth1, amount, bulk.density, goal ){
  A <- cumsum(amount)
  goal.lwr <- goal - 5
  ...
  prior.bd <- bulk.density[index.lwr:index.upr]
...
f(depth0, depth1, amount, goal=c(5, 10, 15, 20, 25, 30) )
...
test <- data %>% group_by(core) %>%
  do({ data.frame( depth=f(.$depth0, .$depth1, .$amount, goal=goal) ) })
