...
points <- st_read("C:/Users/Heidi Rodenhizer/Documents/School/NAU/Schuur Lab/GPS/2019 GPS/spring_2019_post_processing/spring_2019_post_processing.shp/Points.shp") %>%
...
new_names <- as.numeric(names) %>%
  replace(list = which(!is.na(names_num) & names_num < 13000),
          values = c(seq(10001, 10441), seq(11001, 11437), seq(12001, 12440)))

for (i in 1:length(new_names)) {
  if (is.na(new_names[i])) {
    new_names[i] <- new_names[i]
  } else if (as.numeric(new_names[i]) >= 10292 & as.numeric(new_names[i]) <= 10295) {
    new_names[i] <- as.character(as.numeric(new_names[i]) - 41)
  } else if (as.numeric(new_names[i]) >= 10251 & as.numeric(new_names[i]) <= 10288) {
    new_names[i] <- as.character(as.numeric(new_names[i]) + 4)
  } else if (as.numeric(new_names[i]) >= 10296 & as.numeric(new_names[i]) <= 10299) {
    new_names[i] <- as.character(as.numeric(new_names[i]) - 3)
  } else if (as.numeric(new_names[i]) >= 10289 & as.numeric(new_names[i]) <= 10291) {
    new_names[i] <- as.character(as.numeric(new_names[i]) + 8)
  } 
} 

new_names <- new_names %>%
  replace(list = which(is.na(new_names)),
          values = names[1:length(which(is.na(new_names)))])

points <- points %>%
  mutate(Name = new_names)
# st_write(points, "C:/Users/Heidi Rodenhizer/Documents/School/NAU/Schuur Lab/GPS/All_Points/All_Points_May_2019_SPCSAK4.shp")
...
vgm.anis = vgm(0.023, "Exp", 4, 0.002, anis =  c(0, 0.95))
...
# #Write tif files
# writeRaster(A2019re, "C:/Users/Heidi Rodenhizer/Documents/School/NAU/Schuur Lab/GPS/Kriged_Surfaces/Elevation_Variance/Uncorrected_kriged_elevation/A2019K.tif", format = "GTiff")
# writeRaster(B2019re, "C:/Users/Heidi Rodenhizer/Documents/School/NAU/Schuur Lab/GPS/Kriged_Surfaces/Elevation_Variance/Uncorrected_kriged_elevation/B2019K.tif", format = "GTiff")
# writeRaster(C2019re, "C:/Users/Heidi Rodenhizer/Documents/School/NAU/Schuur Lab/GPS/Kriged_Surfaces/Elevation_Variance/Uncorrected_kriged_elevation/C2019K.tif", format = "GTiff")
