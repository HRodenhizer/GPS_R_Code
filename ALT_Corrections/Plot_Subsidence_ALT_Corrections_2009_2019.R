...
filenames <- list.files('C:/Users/Heidi Rodenhizer/Documents/School/NAU/Schuur Lab/GPS/Kriged_Surfaces/Subsidence_Clipped/ALT_Sub_Ratio_Corrected/subsidence_stacks',
                        full.names = TRUE,
                        pattern = '2009-2019')
...
# Load Thaw Data
ALTdata <- read_excel("Z:/Schuur Lab/2020 New_Shared_Files/DATA/CiPEHR & DryPEHR/Thaw Depth/Processed/2019/Cumulative CiPEHR DryPEHR Thaw Data.xlsx",
...
for (i in 1: length(CiPEHR_brick)) {
  ...
  CiPEHR_extract_temp <- raster::extract(CiPEHR_brick[[i]], plotcoords.temp, layer = 11, nl = nlayers(CiPEHR_brick[[i]]), df = TRUE) %>%
    dplyr::select(sub = 2,
                  -ID) %>%
...
for (i in 1: length(DryPEHR_brick)) {
  ...
  DryPEHR_extract_temp <- raster::extract(DryPEHR_brick[[i]], plotcoords.temp, layer = 9, nl = nlayers(DryPEHR_brick[[i]]), df = TRUE) %>%
    dplyr::select(sub = 2,
                  -ID) %>%
    cbind.data.frame(plotcoords.temp) %>%
    mutate(year = 2019,
...
ALTsub <- ALTsub %>%
  rbind.data.frame(ALTsub_2019)

# Write file
# write.csv(ALTsub, 'C:/Users/Heidi Rodenhizer/Documents/School/NAU/Schuur Lab/GPS/Thaw_Depth_Subsidence_Correction/ALT_Sub_Ratio_Corrected/ALT_Subsidence_Corrected_2009_2019.csv', row.names = FALSE)
###########################################################################################################
