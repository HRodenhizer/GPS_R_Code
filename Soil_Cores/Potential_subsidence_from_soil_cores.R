...
get.height <- Vectorize(function( depth0, depth1, amount, goal, goal.lwr ){
  A <- cumsum(amount)
  if( all( A < goal.lwr)){    # if the core isn't deep enough to reach the desired quantity of ash
    return(NA)
  }
...
avg.soil.prop <- Vectorize(function( depth0, depth1, amount, soil.prop, goal, goal.lwr ){
  A <- cumsum(amount)
  ...
  if ( any( A > goal)) {
    ...
    if(length(prior.prop) == 1) {
      weights <- c(goal - goal.lwr)
    } else {
      weights[1] <- (A[index.lwr] - goal.lwr)
      weights[length(weights)] <- need
    }
    prop <- sum(prior.prop * weights) / (goal - goal.lwr)
  } else {
    index.lwr <- min(which(A > goal.lwr)) # layer where we reach the quantity of ash that we are starting from
...
# calculate the alt in units of kg/m^2 of ash and kg/m^2 of soil
alt_ash <- soil_09_17 %>%
  group_by(year, block, fence, treatment) %>%
  ...
  mutate(height = (depth1 - depth0)/100, # convert to meters
         mean.ALT.ash = ifelse(-mean.ALT > depth0 & -mean.ALT < depth1,
                                cu.ash.stock - ash.stock*(depth1 + mean.ALT)/(depth1-depth0),
                                cu.ash.stock + ash.stock*(-mean.ALT - depth1)/(depth1-depth0)),
         mean.ALT.soil = ifelse(-mean.ALT > depth0 & -mean.ALT < depth1,
...
alt_ash_2017 <- alt_ash %>%
  filter(year == 2017) %>%
  ungroup() %>%
  select(block, fence, treatment, mean.ALT.ash.2017 = mean.ALT.ash, mean.ALT.soil.2017 = mean.ALT.soil)
...
goals <- alt_ash %>%
  ...
  mutate(type = paste(type, 'alt', sep = '.')) %>%
  spread(key = type, value = ALT.norm) %>%
  ungroup() %>%
...
soil_09_17_v2 <- soil_09_17 %>%
  filter(year == 2009 & !(fence == 1 & treatment == 'c'))
####################################################################################################################################
...
depth <- soil_09_17_v2 %>%
  do({ data.frame( depth1 = get.height(.$depth0, .$depth1, .$ash.stock, goals.ash, goals.ash.lwr)) })

# average bulk density for each of the previously calculated depths
bd <- soil_09_17_v2 %>%
  do({ data.frame( bd = avg.soil.prop(.$depth0, .$depth1, .$ash.stock, .$bulk.density, goals.ash, goals.ash.lwr))})

# average moisture for each of the previously calculated depths
moisture <- soil_09_17_v2 %>%
  do({ data.frame( moisture = avg.soil.prop(.$depth0, .$depth1, .$ash.stock, .$moisture, goals.ash, goals.ash.lwr))})

rep.goal <- nrow(bd)/length(goals.ash)

# join depth, bd, and moisture dataframes into one
soil_09_17_ash <- depth %>%
  cbind.data.frame(bd = bd$bd, moisture = moisture$moisture) %>%
  mutate(ash.alt = rep(goals.ash, rep.goal),
         depth1 = ifelse(ash.alt == 0,
                         0,
                         depth1),
         bd = ifelse(ash.alt == 0,
                     0,
                     bd),
         moisture = ifelse(ash.alt == 0,
                           0,
                           moisture),
         group.id = group_indices(., block, fence, treatment) + 1,
         goal.id = rep(goal.id.ash, rep.goal)) %>%
  filter(goal.id == group.id) %>%
  select(-goal.id, -group.id) %>%
  mutate(depth0 = insert(depth1[c(TRUE, FALSE)], ats = seq(1, length(depth1)/2), values = 0)) %>%
  left_join(select(goals, year, block, fence, treatment, ash.alt, alt.year),
            by = c('year', 'block', 'fence', 'treatment', 'ash.alt')) %>%
  filter(alt.year == 2017 & !is.na(depth1)) %>%
  mutate(soil.stock = bd*(depth1 - depth0)*10, # bd (g/cm^3) * height (cm) = g/cm^2 * 1kg/1000g * 100 cm/m *100 cm/m = kg/m^2
         ice.height = moisture*soil.stock*1e-6, # moisture (g/kg) * 1 (cm^3 water/g water) *soil stock (kg/m^2) * (1/100 (m/cm))^3 = m
         method = 'ash') %>%
  select(year, block, fence, treatment, method, alt.2009 = depth0, alt.2017 = depth1, bd, moisture, alt.2017.norm = ash.alt, ice.height)

# not sure if this is really useful since the soil core locations are not average for the treatments
soil_09_17_ash_summary <- soil_09_17_ash %>%
  group_by(treatment) %>%
  summarise(mean.ice.height = mean(ice.height),
            se.ice.height = sd(ice.height)/sqrt(n()))
####################################################################################################################################

### calculate the height of cores with the same ('goal') amounts of soil mass in all years ###############################################
# extract depths to chosen uniform soil mass values: (5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55 + the total shared soil mass)
depth <- soil_09_17_v2 %>%
  do({ data.frame( depth1 = get.height(.$depth0, .$depth1, .$soil.stock, goals.soil, goals.soil.lwr)) })

# average bulk density for each of the previously calculated depths
bd <- soil_09_17_v2 %>%
  do({ data.frame( bd = avg.soil.prop(.$depth0, .$depth1, .$soil.stock, .$bulk.density, goals.soil, goals.soil.lwr))})

# average moisture for each of the previously calculated depths
moisture <- soil_09_17_v2 %>%
  do({ data.frame( moisture = avg.soil.prop(.$depth0, .$depth1, .$soil.stock, .$moisture, goals.soil, goals.soil.lwr))})

rep.goal <- nrow(bd)/length(goals.soil)

# join depth, bd, and moisture dataframes into one
soil_09_17_soil <- depth %>%
  cbind.data.frame(bd = bd$bd, moisture = moisture$moisture) %>%
  mutate(soil.alt = rep(goals.soil, rep.goal),
         depth1 = ifelse(soil.alt == 0,
                         0,
                         depth1),
         bd = ifelse(soil.alt == 0,
                     0,
                     bd),
         moisture = ifelse(soil.alt == 0,
                           0,
                           moisture),
         group.id = group_indices(., block, fence, treatment) + 1,
         goal.id = rep(goal.id.soil, rep.goal)) %>%
  filter(goal.id == group.id) %>%
  select(-goal.id, -group.id) %>%
  mutate(depth0 = insert(depth1[c(TRUE, FALSE)], ats = seq(1, length(depth1)/2), values = 0)) %>%
  left_join(select(goals, year, block, fence, treatment, soil.alt, alt.year),
            by = c('year', 'block', 'fence', 'treatment', 'soil.alt')) %>%
  filter(alt.year == 2017 & !is.na(depth1)) %>%
  mutate(soil.stock = bd*(depth1 - depth0)*10, # bd (g/cm^3) * height (cm) = g/cm^2 * 1kg/1000g * 100 cm/m *100 cm/m = kg/m^2
         ice.height = moisture*soil.stock*1e-6, # moisture (g/kg) * 1 (cm^3 water/g water) *soil stock (kg/m^2) * (1/100 (m/cm))^3 = m
         method = 'soil') %>%
  select(year, block, fence, treatment, method, alt.2009 = depth0, alt.2017 = depth1, bd, moisture, alt.2017.norm = soil.alt, ice.height)

# not sure if this is really useful since the soil core locations are not average for the treatments
soil_09_17_soil_summary <- soil_09_17_soil %>%
  group_by(treatment) %>%
  summarise(mean.ice.height = mean(ice.height),
            se.ice.height = sd(ice.height)/sqrt(n()))
####################################################################################################################################
...
### join ash normalized and soil normalized data into one data frame
...
### Calculate potential subsidence using 2009 ice content in the 2009-2017 new active layer ########################################
potential_sub <- soil_core_normalized %>%
  filter(year == 2009) %>%
  left_join(select(filter(soil_core_normalized, year == 2017), block, fence, treatment, type, mass0, mass1, ALT.norm.2017 = mean.ALT.norm), 
            by = c('block', 'fence', 'treatment', 'type', 'mass0', 'mass1')) %>%
  group_by(block, fence, treatment, type) %>%
  rename(ALT.norm.2009 = mean.ALT.norm) %>%
  mutate(ALT.norm.2017 = ifelse(is.na(ALT.norm.2017),
                                first(ALT.norm.2017),
                                ALT.norm.2017),
         depth.category = ifelse()) %>%
  group_by(block, fence, treatment, type, ALT.norm.2009, ALT.norm.2017)
  summarise()
...
g3 <- ggplot(sub_comparison_graph, aes(x = year, y = subsidence, color = source)) +
  geom_point() +
